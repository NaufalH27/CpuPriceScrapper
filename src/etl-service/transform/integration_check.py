def is_integrated(product):
    ...
    if discount_float == None and not isinstance(discount_float, float):
        return False
    if product_img == None or not product_img.startswith("https"):
        return False
    if product_link == None  or not product_link.startswith("https"):
        return False
    if seller_name == None:
        return False
    if seller_location == None:
        return False
    if seller_badge == None or not seller_badge.startswith("https"):
        return False
    if seller_link == None or not seller_badge.startswith("https"):
        return False
    return True
